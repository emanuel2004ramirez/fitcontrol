...
{
    protected $fillable = ['idempotency_key', 'numero_recibo', 'cliente_id', 'metodo_pago_id', 'estado_pago_id', 'monto', 'moneda', 'referencia', 'referencia_externa', 'pagado_at', 'procesado_por', 'observaciones'];

    protected function casts(): array
    {
        return ['monto' => 'decimal:2', 'pagado_at' => 'datetime'];
    }

    public function scopePagados(Builder $query): Builder
    {
        return $query->whereNotNull('pagado_at');
    }

    public function scopeEntreFechas(Builder $query, $desde, $hasta): Builder
    {
        return $query->whereBetween('pagado_at', [$desde, $hasta]);
    }
