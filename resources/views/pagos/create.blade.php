...
<form method="POST" action="{{ route('pagos.store') }}" class="row g-3">@csrf
    ...
    <div class="col-md-8"><label class="form-label">Cliente y membresía</label><select class="form-select" id="membresia" name="membresia_id" required><option value="">Seleccione</option>@foreach($membresias as $m)<option value="{{ $m->id }}" data-total="{{ $m->total }}" data-moneda="{{ $m->moneda }}" @selected(($membresiaSeleccionada??null)==$m->id)>{{ $m->numero_socio }} · {{ $m->cliente }} · {{ $m->tipo }} — {{ number_format($m->total,2) }} {{ $m->moneda }}</option>@endforeach</select></div>
...
@push('scripts')<script>const membresia=document.getElementById('membresia'),actualizarTotal=()=>{const o=membresia.selectedOptions[0];document.getElementById('total').value=o?.dataset.total?Number(o.dataset.total).toLocaleString('es-HN',{minimumFractionDigits:2})+' '+o.dataset.moneda:'Seleccione una membresía';};membresia.addEventListener('change',actualizarTotal);actualizarTotal();</script>@endpush
@endsection
