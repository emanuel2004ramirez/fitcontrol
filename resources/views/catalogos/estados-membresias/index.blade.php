@extends('layouts.app')
@section('title', 'Catálogo de Estados de Membresía')
...
<x-card title="Filtros" class="mb-4">
    <form method="GET" action="{{ route('catalogos.estados-membresias.index') }}" class="row g-3">
        <div class="col-lg-4">
        ...
        <div class="col d-flex align-items-end gap-2">
            ...
            <x-button :href="route('catalogos.estados-membresias.index')" variant="outline-secondary">Limpiar</x-button>
        </div>
    </form>
</x-card>
                ...
                @forelse($estados as $estado)
                    <tr>
                        ...
                        <td class="text-end">
                            ...
                            <form action="{{ route('catalogos.estados-membresias.destroy', $estado->id) }}" method="POST" class="d-inline-block" onsubmit="return confirm('¿Estás seguro de eliminar este registro?');">
                                @csrf
                                @method('DELETE')
                    ...
                    <div class="modal fade" id="modalEditar{{ $estado->id }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form action="{{ route('catalogos.estados-membresias.update', $estado->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div class="modal-body text-start">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="codigo" class="form-control" value="{{ $estado->codigo }}" required maxlength="255">
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Nombre <span class="text-danger">*</span></label>
                                            <input type="text" name="nombre" class="form-control" value="{{ $estado->nombre }}" required maxlength="255">
                                        </div>
                                        <div class="mb-3">
        ...
        <div class="modal-content">
            <form action="{{ route('catalogos.estados-membresias.store') }}" method="POST">
                ...
                <div class="modal-body text-start">
                    <div class="mb-3">
                        <label class="form-label">Código <span class="text-danger">*</span></label>
                        <input type="text" name="codigo" class="form-control" required maxlength="255">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Nombre <span class="text-danger">*</span></label>
