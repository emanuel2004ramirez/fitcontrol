@extends('layouts.app')
@section('title', 'Catálogo de Estados de Pago')
...
<x-card title="Filtros" class="mb-4">
    <form method="GET" action="{{ route('catalogos.estados-pagos.index') }}" class="row g-3">
        <div class="col-lg-4">
        ...
        <div class="col d-flex align-items-end gap-2">
            ...
            <x-button :href="route('catalogos.estados-pagos.index')" variant="outline-secondary">Limpiar</x-button>
        </div>
    </form>
</x-card>
                ...
                @forelse($estados as $estado)
                    <tr>
                        ...
                        <td class="text-end">
                            ...
                            <form action="{{ route('catalogos.estados-pagos.destroy', $estado->id) }}" method="POST" class="d-inline-block" onsubmit="return confirm('¿Estás seguro de eliminar este registro?');">
                                @csrf
                                @method('DELETE')
                    ...
                    <div class="modal fade" id="modalEditar{{ $estado->id }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form action="{{ route('catalogos.estados-pagos.update', $estado->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
