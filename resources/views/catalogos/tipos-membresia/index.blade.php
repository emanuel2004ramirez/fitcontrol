@extends('layouts.app')
@section('title', 'Catálogo de Tipos de Membresía')
@php($breadcrumbs = [['label' => 'Catálogos'], ['label' => 'Tipos de Membresía']])
@section('content')

<x-page-header title="Tipos de Membresía" subtitle="Catálogo de tipos y precios de membresía del sistema.">
    <x-button data-bs-toggle="modal" data-bs-target="#modalCrear" icon="bi-plus-lg">Nuevo Tipo de Membresía</x-button>
</x-page-header>

<x-card title="Filtros" class="mb-4">
    <form method="GET" action="{{ route('catalogos.tipos-membresia.index') }}" class="row g-3">
        <div class="col-lg-4">
        ...
        <div class="col d-flex align-items-end gap-2">
            ...
            <x-button :href="route('catalogos.tipos-membresia.index')" variant="outline-secondary">Limpiar</x-button>
        </div>
    </form>
</x-card>
            ...
            <tbody>
                @forelse($tipos as $tipo)
                    <tr>
                        ...
                        <td>{{ number_format($tipo->precio ?? 0, 2) }}</td>
                        <td>{{ $tipo->moneda ?? 'N/A' }}</td>
                        <td>
                            @if($tipo->activo)
                        ...
                        <td class="text-end">
                            ...
                            <form action="{{ route('catalogos.tipos-membresia.destroy', $tipo->id) }}" method="POST" class="d-inline-block" onsubmit="return confirm('¿Estás seguro de eliminar este registro? Se eliminarán todos sus precios históricos.');">
                                @csrf
                                @method('DELETE')
                    ...
                    <div class="modal fade" id="modalEditar{{ $tipo->id }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form action="{{ route('catalogos.tipos-membresia.update', $tipo->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <div class="modal-header">
                                        <h5 class="modal-title">Editar Tipo de Membresía</h5>
                                    ...
                                    <div class="modal-body text-start">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea name="descripcion" class="form-control" rows="2">{{ $tipo->descripcion }}</textarea>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-4 mb-3">
                                                <label class="form-label">Días <span class="text-danger">*</span></label>
                                                <input type="number" name="duracion_dias" class="form-control" value="{{ $tipo->duracion_dias }}" required min="1">
                                            </div>
                                            <div class="col-md-4 mb-3">
...
<div class="modal fade" id="modalCrear" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('catalogos.tipos-membresia.store') }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title">Nuevo Tipo de Membresía</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                </div>
                <div class="modal-body text-start">
                    <div class="row">
