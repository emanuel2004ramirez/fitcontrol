...
<div class="row g-3 g-xl-4 mb-4">
 ...
 <div class="col-6 col-xl-3"><x-stat-card label="Cuentas por cobrar" :value="number_format($indicadores->cuentas_por_cobrar,2).' HNL'" icon="bi-receipt" tone="warning"/></div>
...
new Chart(document.getElementById('membershipChart'),{type:'doughnut',data:{labels:@json(collect($membresiasEstados)->pluck('estado')),datasets:[{data:@json(collect($membresiasEstados)->pluck('cantidad')),backgroundColor:['#2563eb','#10b981','#f59e0b','#ef4444','#8b5cf6','#06b6d4'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,cutout:'68%',plugins:{legend:{position:'bottom',labels:{boxWidth:10,usePointStyle:true}}}}});
</script>
@endpush
