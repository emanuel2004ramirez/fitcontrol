...
@push('scripts')<script>
document.addEventListener('DOMContentLoaded',()=>{const modal=new bootstrap.Modal('#expedienteModal');modal.show();const pasos=[...document.querySelectorAll('.wizard-step')],ant=document.getElementById('anterior'),sig=document.getElementById('siguiente'),guardar=document.getElementById('guardar'),texto=document.getElementById('pasoTexto'),barra=document.getElementById('pasoBarra');let actual=0;function pintar(){pasos.forEach((p,i)=>p.classList.toggle('d-none',i!==actual));ant.classList.toggle('d-none',actual===0);sig.classList.toggle('d-none',actual===pasos.length-1);guardar.classList.toggle('d-none',actual!==pasos.length-1);texto.textContent=`Paso ${actual+1} de ${pasos.length} · ${pasos[actual].dataset.title}`;barra.style.width=`${((actual+1)/pasos.length)*100}%`;}sig.addEventListener('click',()=>{const campos=[...pasos[actual].querySelectorAll('input,select,textarea')];if(campos.some(c=>!c.reportValidity()))return;actual++;pintar();});ant.addEventListener('click',()=>{actual--;pintar();});pintar();});
</script>@endpush
@endsection
